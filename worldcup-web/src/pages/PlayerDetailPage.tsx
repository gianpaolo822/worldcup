...
import { calcAge, positionLabel } from '@/lib/playerUtils';
  ...
  return (
    <>
      <section className="sb-card-elevated p-5 mt-4 mb-4 text-center">
        <div
          className="w-16 h-16 rounded-full mx-auto mb-3 flex items-center justify-center text-xl font-bold border-2"
          style={{
        ...
        <h2 className="text-lg font-bold text-[var(--text)]">{player.name}</h2>
      ...
      <section className="sb-card p-4 mb-4">
        ...
        <InfoRow label="俱乐部" value={player.club} />
      </section>

      {player.injuryNote && (
        <section className="sb-card p-4 mb-4">
          <p className="sb-label mb-2">伤病说明</p>
          <p className="text-sm text-[var(--text-muted)]">{player.injuryNote}</p>
        </section>
      )}
...
export default function PlayerDetailPage() {
  ...
  const title = squadPlayer?.name ?? historicalScorer?.name ?? '球员详情';
